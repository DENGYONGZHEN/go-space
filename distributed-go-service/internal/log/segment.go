...
import (
	...
	api "distributed-go-service/api/v1"

	"google.golang.org/protobuf/proto"
)

type segment struct {
	store                  *store //保存实际序列化后的数据
	index                  *index //保存 offset → position 的映射（用 mmap）
...
func (s *segment) Append(record *api.Record) (offset uint64, err error) {
	...
	//再把相对offset和store中的位置写入index文件中，相对offset是可以是索引从0开始
	if err = s.index.Write(uint32(s.nextOffset-uint64(s.baseOffset)), pos); err != nil {
		return 0, err
	}

	s.nextOffset++
	//返回此segment所存储的全局offset
	return cur, nil
}

func (s *segment) Read(off uint64) (*api.Record, error) {
	//传入的off是全局offset，需要获取相对的offset，然后查找索引，获取在store中的位置
