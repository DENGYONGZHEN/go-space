...
func (r *Registrar) Init() error {
	cList, err := r.DockerClient.ContainerList(context.Background(), container.ListOptions{
...
func (r *Registrar) Observe() {
	msgCh, errCh := r.DockerClient.Events(context.Background(), events.ListOptions{
		Filters: filters.NewArgs(
	...
	for {
		select {
		case c := <-msgCh:
			fmt.Printf("State of the contianer %s is %s\n", c.ID, c.Status)
			if c.Status == ContainerKillState {
				...
				if err != nil {
					fmt.Printf("err geeting newly started container port %s\n", err.Error())
					continue
				}
